            ...
            <ul class="course_details">
                ...
                <li class="temps_arrivee">
                    ...
                    @if ($course->heure_arrivee)
                        {{ \Carbon\Carbon::parse($course->heure_arrivee)->format('H') }}h {{ \Carbon\Carbon::parse($course->heure_arrivee)->format('i') }}min
                    @else
                        Non spécifiée
                    @endif
                </li>
                    ...
                    @if (!isset($course->est_facture))
                            <div class="review-form">
                                <form action="{{ route('courses.submitReview', $course->id_course) }}" method="POST" class="reviewForm">
                                    ...
                                    <input id="champPourboire" name="pourboire" type="number" min="0" step="1" max="{{ $course->prix_reservation }}" value="0">
                        ...
                        @else
                            <form action="{{ route('courses.Facture', $course->id_course) }}" method="POST" style="display:inline" class="formGenereInvoice">
                                ...
                                <button class="generateInvoiceButton" type="submit" target="_blank">Générer ma facture</button>
                            </form>
                        @endif
                    @else
                        @if (is_null($course->acceptee))
                            <form action="{{ route('courses.update', ['id' => $course->id_course]) }}" method="POST" id="formCourseListe">
                                @csrf
                                @method('PUT')
                                <button class="butMpageClient modifyButton" type="submit" data-course-id="{{ $course->id_course }}">Modifier</button>
                            </form>
                        ...
                        @elseif ($course->acceptee === true)
                            <form action="{{ route('client.terminer', $course->id_course) }}" method="POST" style="display:inline">
                                @csrf
                                @method('PUT')
                                <button class="acceptButton butTpageClient" type="submit" onclick="return confirm('Êtes-vous sûr de vouloir terminer cette course ?')">Terminer</button>
                            </form>
                            <form action="{{ route('courses.update', ['id' => $course->id_course]) }}" method="POST" id="formCourseListe">
                                @csrf
                                @method('PUT')
